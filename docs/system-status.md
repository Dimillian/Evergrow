...
Sound audition foundation (2026-09-06): `/sounds.html` offers 26 authored procedural candidates across weapons/material impacts, magic, creatures, rewards and utility, with six reproducible variants, per-sound tuning, dry/woodland/crypt acoustics, loop/stop, WAV export and copyable settings. This is an approval/iteration tool; current gameplay still uses `GameAudio` pending palette selection and typed weapon/element/creature event integration. See [Sound studio](sound-studio.md).

The sound audition palette now uses 32 locally bundled CC0 material recordings and performed vocals across its 26 cues, replacing the first synthetic candidates. Source credits and file mappings live in `game/src/assets/audio/`; natural sample attacks/decay, six variations and room/tuning controls are shared through `sound-library.ts` and `sound-engine.ts`. This remains a local review tool; live gameplay audio awaits palette approval.

Listening correction: level-up and rare-item pickup use their initial synthesized studio recipes again, as requested; the rest of the sample-led audition palette remains.
