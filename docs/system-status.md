...
Local September 19 character-window split: P opens identity, progression, attributes and Offense / Defense / Resources tabs; I opens equipment, pack, charms and quick skill assignment. Windows switch through shortcuts or footer links while paused. Headers have no shortcut letters beside Close. Town portal moves to R; unsupported combined-action custom key maps fall back to the new defaults. No character-save or progress reset.
...
| Town portal | Free three-second R channel, home-town anchors, saved single-use return endpoint, safe landing, native control/map markers and arrival protection | Permanent waypoint network and map travel remain specified |
