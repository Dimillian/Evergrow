...
Champion/elite emission now stays readable after ambient lighting: blue/gold rank glow and ground rings, with distinct cyan speed trails (Swift), amber sparks (Relentless), red claw marks (Savage), and violet ward rings (Resolute). Rank health bars remain visible at full life. Target plates separate the colored modifier names from the monster name, and persistent modifier icons share the existing hoverable effect strip with exact mechanical descriptions. Bosses retain their own authored appearance. These signatures use bounded strokes/cached glows, viewport culling, no extra particles, no additional dynamic lights, and no second character-render pass. Reduced motion freezes rank effects and replaces animated arrival bolts with the static seal/glow.
