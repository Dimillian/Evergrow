...
Journey **Show on map** uses the same motion and arrival highlight at the objective. The map first opens on the player with a 180 ms hold, then pans to the public objective marker at the current zoom; undiscovered objectives retain their coarse search-area position.
