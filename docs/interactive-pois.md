...
Active cursed chests display a violet seal above the chest: the ring drains with the recipe's remaining time and its center shows seconds left. It reads the saved trial clock, waits for admission and pauses with gameplay; it owns no event progression or rewards. The indicator and text render at native resolution after world post-processing.

Local review: `/events.html`. All thirteen interaction families use real frozen scenes. **Preview In Progress** is available for cursed chests, with Play/Pause, an elapsed-time scrubber, 1×/5×/10× speed and Restart. `/events.html?view=cursedChest&state=progress` opens it directly. The disposable trial uses the runtime recipe duration and indicator; no enemies or waves are simulated. Finishing the countdown replays the existing sample six-wave reward opening. Scrubbing pauses for inspection, hidden tabs suspend the countdown, and reduced motion starts paused. **Preview opening** advances presentation only and shows a six-wave cursed payout; neither mode ticks combat or accesses saves.
