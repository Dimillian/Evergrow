...
World → **Event choices** (`/events.html?choices`) previews the selected action-row layout shared with the runtime panel. Every trial recipe, caravan equipment/gold and the seeded blessing pair use the same `event-choice-presentation.ts` content/markup and `poi-panel.css`. Objective and reward details remain in the body and action rows; there are no bottom footnotes. Level 12 sample events sit over frozen runtime scenery, with Desktop/390px views, dismissal and reset in disposable memory. `scenario=cursedChest:0|caravan:0|standingStones:0|…` and `size=desktop|narrow` are URL-addressable. The unused comparison-card and briefing proposals have been removed.
...
- **Characters**: shared appearance editor, animated atelier, eight-facing rig, in-world looks, hair/accessory catalog, phone study and character hall.
  ...
  The eight-facing rig (`/rig.html`) includes simple clothing / plate-and-cloak, helmet visibility, all 17 weapon profiles, unarmed and paired equipment fixtures. Sword/shield uses the actual one-handed Longsword. Focus isolates a direction; Joint overlay exposes shared arm and leg joints. Phase scrubs walking, basic attacks, casting, thrusts, slams and shield bashes; Travel adds backward and sideways strides. Controls persist in the URL. It defaults to unarmed with the shared default appearance. All directions and phases use one framing envelope for the selected loadout/action, so scrubbing never zooms the character. Frozen presentation only, without gameplay or saves.

  The appearance catalog (`/appearance-catalog.html?kind=hair`) compares all 24 hairstyles over three pages. Directions switches between four cardinal views and all eight facings; Hair color exposes the runtime palettes and Helmet checks coverage. These choices persist in the URL and across catalog pages (`directions=8&color=flax&helmet=0`). Full figure (`figure=1`) adds the shared unarmed body to inspect hair against the neck and shoulders. Head detail uses the production head renderer; both modes stay independent of character saves.
...
### Command console study

Interface → Command console (`/tools/console.html`) uses the production command panel over a frozen runtime-rendered world. Discover, Item drop, Monster, Arguments and Refill presets demonstrate contextual suggestions and utility input. Commands in this study only show demonstration receipts; no character saves, simulation updates or game mutations are connected. The production local implementation and command syntax are documented in [Local command console](local-console.md). The study remains outside production entrypoints; the actual console is conditionally included only in local builds.
