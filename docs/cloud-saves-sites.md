...
**Delete** also works during a conflict. Its confirmation explicitly deletes both the cloud save and this device’s recovery copy. With the local September 12 fix, every cloud deletion requires a connection and compares the latest server revision before deleting. Recovery remains intact until the server acknowledges deletion. A failed request or concurrent server save leaves recovery available and shows a retry message; a concurrent recovery edit is retained. This deletion fix was published in v0.3.2.
...
### Cloud recovery and deletion — local, September 12, 2026
