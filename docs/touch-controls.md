...
The colored navigation trigger and Pause form a vertical column above Interact/Portal on the movement-disc axis. The destination list opens beside the trigger and is height-bounded when landscape space is short. The mini journey tracker is omitted in this layout; a reduced translucent minimap remains visible for orientation. One compact pickup notification is shown below the target plate. Safari visual-viewport changes resize the shell, world buffer, native UI and touch geometry together. The static touch study uses the same presentation policy and layout. These are code-checked bounds, not a claim of physical iPhone acceptance.
