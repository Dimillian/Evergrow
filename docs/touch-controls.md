...
A single top-right menu toggle reveals the existing shortcuts; map, inventory, skills and journeys remain accessible. The desktop minimap and journey tracker are omitted in this layout. One compact pickup notification is shown below the target plate. Safari visual-viewport changes resize the shell, world buffer, native UI and touch geometry together. The static touch study uses this same layout. These are code-checked layout bounds, not a claim of physical iPhone acceptance.
