...
Outdoor combat silhouettes use `enemy-occlusion.ts` for eligibility, bounds and short presentation fades. Nearby living enemies in chase/windup/attack/recovery receive a warm-red fill and stronger edge only where foreground prop pixels cover their current interpolated body/weapon pose. Strength tapers from 600 to 720 world units; indoor enemies and dungeon scenes are excluded. `prop-sprite-art.ts` shares actual wind transforms and player canopy opacity between the scene and coverage masks. `enemy-occlusion-art.ts` reuses three actor-sized scratch canvases with no pixel readback or actor-count cap; overlapping foliage combines before one reveal is drawn. Foreground architecture, roofs and character bodies mask the reveal, including walls behind foliage. The pass follows surface lighting and precedes effects/CRT; profiler time belongs to actors. Reduced motion applies fade targets immediately. No enemy detection, targeting, collision, saves or combat rules change.
