...
`canopy-visibility.ts` extends the existing player canopy fade to nearby engaged outdoor enemies. Once per frame it selects living combatants within 720 world units whose interpolated body bounds intersect the displayed viewport, excluding indoor enemies. Each tree uses numeric depth/body/crown overlap checks against those candidates, then its existing foliage layers fade to 24% opacity with the same smoothing and reduced-motion behavior as player cover. Trunks, rocks, walls and roofs keep their normal rendering. No additional actor rendering, alpha-mask surfaces or compositing pass is used. Dungeon scenes remain unchanged; travel/full resets clear the existing crown-opacity state while retaining PR #68's travel cache reuse. Enemy detection, targeting, collision, saves and combat rules are unchanged.
