...
The **Storage** chest beside each hearth exposes the same **96-item personal stash**, separate from the carried bag. It is per character, not an account-wide item exchange. Select carried gear and Store item, or stored gear and Take item. Stored gear uses the same 12-column grid, physical item footprints, art and tooltips as inventory. Cells shrink to fit the panel without horizontal scrolling; the grid starts with eight rows and expands vertically as needed to retain the 96-item capacity. Stored charms keep their physical size but grant no bonuses. Visual packing preserves saved slot identities for retrieval. Independent one-click Auto-sort buttons organize the chest and carried inventory through character commands; sorting clears pending item selections, preserves item properties and saves the new order. Transfers retain exact item identity, recipe and enhancements, never spend gold and never change equipment. The existing service phase supplies Escape/controller closing and the durable transaction boundary.

Optional `CharacterSheet.stash` travels in the ordinary character checkpoint, local/worker/cloud validation and existing save cadence. Validation checks all 96 cells and uniqueness against bag, equipment, buyback and ground items across locations. Failed saves leave both containers unchanged. No new cloud endpoint or database migration is needed.
