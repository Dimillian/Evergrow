...
The **Storage** chest beside each hearth exposes the same **96-slot personal stash**, separate from the 64-slot carried bag. It is per character, not an account-wide item exchange. Select carried gear and Store item, or stored gear and Take item. Shared item cells/tooltips keep presentation consistent with inventory. Transfers retain exact item identity, recipe and enhancements, never spend gold and never change equipment. The existing service phase supplies Escape/controller closing and the durable transaction boundary.
