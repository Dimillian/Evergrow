...
The supplied Chrome captures showed a change from 59.6 FPS to 32.5 FPS. In the slower capture, median rendering CPU was 21.1 ms with no enemies, projectiles or ground effects; the slowdown continued after terrain loading. This identifies rendering as the dominant measured cost, but does not establish an accumulating leak or prove that resource reuse fixes the sustained slowdown.

Run `CANVAS_MODULE=/absolute/path/to/@napi-rs/canvas node --experimental-strip-types game/scripts/benchmark-travel-rendering.mjs` from the repository root. This frozen, save-free comparison draws the actual renderer at town/wilderness destinations using both full and travel resets, counts new canvases and compares every output pixel. It exercises the Canvas fallback, not browser workers or GPU timing. At 960 × 600, repeated town returns previously allocated 169 canvases; the travel reset allocated zero in the same comparison. GPU/worker lifecycle and arrival coverage also have code regressions. Browser FPS acceptance remains the player's test.
...
Freeze holds the displayed capture while gameplay and profiling continue; Resume returns to current samples. Reset starts a fresh capture. Export JSON downloads the frozen capture or current timeline, percentiles and ten slow CPU frames. Version 2 also records each frame's phase (including NPC role) and up to 1,800 one-second summary buckets, split at phase changes, to retain earlier session comparisons after the 600-frame raw timeline wraps. NPC/inventory rebuild and animation CPU outside the game callback is included once in the next frame under **NPC / inventory**. This does not measure browser GPU or compositor work. See [panel performance](panel-performance.md). Reports contain timings, phase labels and scene counts, not character checkpoints. The panel uses native-resolution Canvas graphs and the shared UI frame/fonts above world post-processing. Its controls consume pointer/keyboard input without pausing gameplay; Escape closes it when one of its controls has focus. F3 respects the current performance-overlay binding. Its visible DOM rectangle is projected into the shared logical UI hit region, including canvas-captured drags and navigation-hidden layouts. Coordinate-based entry clears held input and queued simulation actions; closing the monitor removes its hit region.
