...
Browser Options offers `Display`, `60 FPS` and `30 FPS` presentation limits. The same policy covers the game shell and title portrait, with independent pacer state for their independent animation loops. Android remains fixed at 60 FPS and does not expose this control. The cap stays at the start of the main frame loop, so lowering it also lowers input polling and `Simulation.update` call cadence while the simulation continues to use its fixed 120 Hz steps. The runtime bounds an accepted frame delta to 50 ms; delayed frames beyond that boundary can discard elapsed time. See [the frame-limit benchmark](frame-limit-benchmark.md) for the acceptance and measurement protocol.
