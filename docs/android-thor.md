...
Both Android windows request a matching-resolution 60 Hz display mode. The main game and title portrait use independent frame pacers under the same fixed Android policy to cap presentation at 60 FPS if the OS keeps a higher refresh rate. Combat continues to use its fixed 120 Hz simulation steps, although the accepted runtime delta remains bounded. Browser builds may offer Display, 60 FPS and 30 FPS presentation limits; that preference does not override Android and is omitted from its Options window. On the connected Thor, Android confirmed the main panel switched to 60 Hz; the lower panel remained physically at 120 Hz despite registering the companion’s 60 Hz request. The companion still receives state at 4 Hz and map images at 2 Hz; it does not run a second game simulation.
