...
Native `KeyEvent` / joystick `MotionEvent` input is normalized into the same standard snapshot consumed by `GamepadInput`. Existing dead zones, action mappings, neutral rearm, inventory/menu navigation and disconnect pause apply. See [controls](controls.md). On the title screen, D-pad/left stick highlights characters and A loads the highlighted save directly; A on an empty slot focuses creation. Character names use the Android keyboard. D-pad left opens Character stats; D-pad right opens Inventory. The upper inventory window has an LB/RB rail for Equipment and Inventory; Character uses LB/RB for Offense, Defense and Resources. Both have footer links for switching without resuming. The atlas has Tree / Node / Skills sections, a pinned skill bar, A for node actions, X to reach assignment, and right-stick inspector scrolling. Its compact effect summary and assignment remain near the top; no touchscreen is needed to assign skills.
