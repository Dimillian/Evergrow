...
| Right mouse / 1–4 | Use assigned skills; empty slots open assignment |
...
Unlock skills in the tree, then assign them to RMB or 1–4. Click an empty HUD slot, tap an empty touch skill button, or press its keyboard/controller binding to open the anchored skill picker. It lists only unlocked skills usable with current equipment and not assigned elsewhere. If none qualify, the empty slot does nothing and combat continues. Select a skill to save its assignment without casting; Escape/B closes the picker. Character, inventory, skill tree, Journeys, map and town-service panels pause combat.

In shops, select an item and Buy/Sell, or Shift-click for a direct trade. The Equipped section at the blacksmith or enchanter upgrades worn gear in place. Stock and bag use the same Sort & filter controls as the player inventory. With no item selected, Sell… opens All Common, Magic and lower, Rare and lower, Epic and lower, and All. Choosing a row immediately sells every matching bag item, including items hidden by browsing filters; equipment is excluded. Click the selected item again to clear selection. Escape/B closes a popup first, then the service.
