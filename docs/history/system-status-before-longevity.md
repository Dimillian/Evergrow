# Historical system status before world-state consolidation

Snapshot preserved 2026-09-06. Counts and next-work statements below are historical; use [current status](../system-status.md).
...
| Wilderness cells / camp ledger | 128 cached cells / 1,024 persisted camps, up to sixteen members each |
...
Goblin warbands (2026-09-06): one third of generated nonstarter camps hold 10–15 Scrap Goblins and a veteran/elite War Chief. Horn warnings precede alternating rush (+20% movement/damage) and surround orders. Killing the chief breaks morale for 2.2 seconds. At that checkpoint the shared population ceiling was 32, reserving nine roaming slots; the current tuning is listed in the Spawning row above. Goblins have lower individual XP and gear/gold yields; chiefs use normal rank reward tables. Camp casualties and strongbox completion use the existing persistence. No new boss rank or save schema is introduced.
