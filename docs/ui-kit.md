...
The [Loot log](loot-log.md) adds a shared Astral history window with newest-first pickup rows and the existing item detail/term presentation. Its button sits under the transient pickup feed, and the pause menu’s Character category provides keyboard/controller access and restores focus to its Loot log entry on return. Opening it uses `PanelCoordinator`; historical items have no mutation actions.
