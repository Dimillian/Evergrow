...
Tooltip surfaces are owned by `tooltip-material.css`, shared by item cards, generic tooltips and shortcut hints. Item comparison cards use opaque smoked surfaces and local rarity gradients, with no backdrop filter; the group owns only layout and motion. Their brief engraving glint releases its animation state when finished. Generic tooltips and shortcut hints retain their existing glass treatment. Moving among SVG paths or labels inside the same inventory/vendor slot does not rebuild its tooltip. Each item comparison computes one equipment preview and shares it between displacement selection and the primary card’s stat rows; Alt/Shift comparisons retain their existing behavior.
