...
Ground-loot inspection follows the mouse with a 16-pixel gap. It uses the shared equipment preview: the candidate keeps its original property rows with green (+x) or red (-x) net changes beside each value. Additional lost or derived stats retain their own compact change rows. Adjacent cards identify displaced gear with a small Equipped indicator beside the item type; its title names the slot. There is no extra comparison column or heading row. Two-handed swaps include both displaced hands. Empty slots have no equipped card; unavailable equip plans show their reason instead of a misleading comparison. The group flips left or above near screen edges and stays within a 16-pixel viewport margin, keeping the candidate nearest the pointer and stacking cards on narrow screens. Canvas pointer coordinates are converted to CSS pixels so scaled game views retain correct placement. The passive cards do not intercept clicks; selected walking targets keep only their ground highlight when the pointer leaves.
