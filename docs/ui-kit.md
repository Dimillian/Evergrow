...
The [Loot log](loot-log.md) adds a shared Astral history window with newest-first pickup rows and the existing item detail/term presentation. Its button sits under the transient pickup feed, and the pause menu provides controller access. Opening it uses `PanelCoordinator`; historical items have no mutation actions.
