...
Whenever the simulation is paused, the world transitions to black and white, including while inventory, skills, services, Journeys, Chronicle, the full map or the shortcut menu is open, and during durable action holds. Game uses the same activity gate for simulation updates and the presentation-only `pause-transition.ts` clock. In `PostFX`, an aspect-correct circular wave expands from the center over one second, smoothly leaving grayscale behind; resuming retracts it over 750ms so color returns from the edges inward. Reversal continues from the current radius. The effect runs after CRT glow and damage tint, before the native HUD and menus, which retain their colors. Simulation pauses/resumes immediately, independently of the visual transition. OS reduced motion switches colors immediately; the non-WebGL fallback uses a uniform grayscale fade. Static interface reviews pass the finished pause amount directly to the same renderer.
