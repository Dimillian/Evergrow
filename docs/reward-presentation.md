...
Settled coin piles make short, sequenced 3–5 world-unit hops roughly every 4–6 seconds, with seeded timing and height variation. Mana vials share the quiet hop at 80% height and have a soft blue backglow with brighter blue liquid. Health vials have a matching soft red backglow. Shadows stay on the ground; health vials keep their existing motion. Reduced motion disables hops while retaining both resource halos. `drop-idle-motion.ts` owns presentation timing without consuming gameplay RNG or changing pickup/save coordinates. Initial coin scatter and treasure flights remain authoritative until settled. Preview in Equipment → Coins & mana drops (`/loot.html?resources`).
