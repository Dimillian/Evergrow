...
- Newly collected ground items carry a soft inner frame with two slowly twinkling corner glints in the character inventory, including charms and overflow. The frame stays visible between twinkles. Hovering, keyboard/controller focus or touch inspection clears that item's marker. Sorting, closing the window, equipping and saving preserve uninspected markers; inspection never changes Recent sorting. Reduced motion uses still glints. Existing owned items start without markers, and no save reset is needed.
- Inspections during a pending durable save are queued by player session and item ID. After that operation succeeds or fails, still-owned inspected items are acknowledged on the current sheet and checkpointed together; no second hover or tap is needed. Switching characters or leaving ownership behind cannot acknowledge a different item.
