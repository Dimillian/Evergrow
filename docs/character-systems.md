...
- **C:** opens Character: name, equipped portrait, level/XP, attribute allocation and detailed Offense / Defense / Resources tabs. Click an attribute + button to spend one point; Shift-click spends up to ten available points in one action. Defense includes elemental resistances; Resources includes utility and efficiency; Offense includes bonus skill ranks.
- **I:** opens Equipment & inventory: the equipped doll and eleven slots beside the 12×6 spatial pack and four dedicated charm rows. The **Stats** tab attached to the left edge opens an optional compact drawer, hidden by default, with all four attributes, allocation controls and a scrolling breakdown of the same substats and explanations as Character. The inventory frame and its contents stay fixed while stats slide out; where left-side space is limited the drawer overlaps the inventory’s left side. Values update immediately after equipment or attribute changes. The toggle is remembered during this game session without changing saves. Item actions, appearance editing and quick skill assignment remain here. Footer links switch between the windows while paused.
...
The character window exposes hover and keyboard-focus explanations for every attribute and combat stat. Each compact, square-cornered glass tooltip presents the result first, a short effect, then icon-led source sections with aligned values: Base & allocated, Equipment, Charms, Skill tree and Active effects. Empty sections are omitted. Starting and assigned attribute points appear as separate rows; the redundant attribute formula banner is removed. Derived stats retain their calculation and cap/timing notes beneath the sources. Individual equipped items retain their names, with shared gear-slot icons replacing slot suffix text; accessible labels and hover titles identify the slot. Only eligible charms appear. Bag items contribute nothing. Weapon damage and cadence come from `deriveAttackStats`; per-hand damage includes only that weapon's elemental enchantment. Paired weapon rates are separate, never summed. Staff/wand rates are labeled casts, including the off hand.
