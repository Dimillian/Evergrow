...
- **I:** opens Equipment & inventory: the equipped doll and eleven slots beside the 12×6 spatial pack and four dedicated charm rows. The **Stats** tab attached to the left edge reveals an optional compact left column, hidden by default. The narrower portrait leaves room to see gear alongside all four attributes, allocation controls and a scrolling breakdown of the same substats and explanations as Character. Values update immediately after equipment or attribute changes. The toggle is remembered during this game session without changing saves. Item actions, appearance editing and quick skill assignment remain here. Footer links switch between the windows while paused.
