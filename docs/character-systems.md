...
- The filter icon beside **Inventory** opens **Sort & filter**. **Auto Equip** sits beside **Gear power** in the Equipment heading, including the mobile Equipment tab.
- **Auto Equip** compares complete, valid loadouts using derived character stats. Weapons, gloves and orbs favor damage; shields, chest and legs favor protection; boots favor movement; headgear and grimoires favor mana; cloaks favor sustain and cooldowns. Jewelry balances these benefits. The estimate includes weapon cadence, critical hits, alternating hands, capped armor/resistances, block and five seconds of regeneration/on-hit healing. It uses proportional gains with extra weight for the slot's role, rather than item power or rarity. Unique powers, individual skill rotations and utility affixes such as gold find are not modeled; review those tradeoffs manually.
- A better main weapon of a different family or handedness opens a warning showing both weapons: **Equip anyway** allows the replacement and safe offhand stowing; **Keep current weapon only** retains the exact main weapon and upgrades other gear; **Cancel** changes nothing. Same-type upgrades need no warning. Plans consider all equipment in the bag regardless of filters, respect level and bag capacity, preserve ties and commit together. Existing offhand types are preserved, both ring slots participate, and charms remain in their separate active grid. Auto Equip reports the upgraded slot count or that no upgrade is available through the shared notification feed; action results appear immediately ahead of queued pickups.
...
The character pack is now **12 columns × 6 rows (72 cells)**, followed by **four charm rows**. The [charm system](charms.md) now activates magical stones in those rows; equipment cannot enter them. This replaces the earlier one-item-per-cell bag described above; it is local and unpublished.
...
- Optional `inventoryLayout` maps owned item IDs to top-left pack cells. Item records retain their stable bag indices during dragging. Older 64-entry records remain readable, grow to 120 records when necessary, and are packed deterministically without modifying the original on read. Items that cannot fit remain accessible in an overflow tray; new pickups for that inventory region are blocked until its overflow is cleared. No item is deleted or transferred to a different character. Save validation rejects overlaps, edge crossings, non-owned IDs and placement in the wrong inventory region.
...
Inventory docks the shared HUD in its own compact footer, restoring space to the equipment, bag and stat columns. It hides the orb utility controls and menu medallion and replaces the duplicate inventory XP bar. Square skill tiles and corner key badges are shared with the shorter gameplay HUD. Its health/mana orbs and current/maximum numbers update immediately when attributes or equipment change. Left-click an assigned skill slot for the full atlas and its details; right-click for a compact picker anchored above that slot. All five bindings (RMB, 1–4) accept any unlocked skill, with missing equipment marked. Selecting a skill assigned elsewhere moves it; Clear empties the slot. LMB stays the equipped weapon's basic attack. Escape dismisses the picker first. See [skill progression](skill-progression.md) for details.
