...
function setup() {
  ...
  const coordinator = new PanelCoordinator({ skillAssignment: panel('skillAssignment'), journeys: panel('journeys'), event: panel('event'), service: panel('service'), map: panel('map'), character: panel('character'), skills: panel('skills') }, {
...
test('all registered panels clear held movement/actions and simulation velocity on entry and resume', () => {
  for (const name of ['map', 'character', 'skills', 'skillAssignment', 'service', 'event'] as PanelPhase[]) {
...
test('title and defeat close every active panel without returning focus to gameplay', () => {
  for (const next of ['ready', 'dead'] as const) for (const name of ['map', 'character', 'skills', 'skillAssignment', 'service', 'event'] as PanelPhase[]) {
