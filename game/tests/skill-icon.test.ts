...
test('every active skill has bounded immutable relief geometry and distinct small-size art', () => {
  ...
  for (const { id } of Object.values(SKILL_DEFINITIONS)) {
    ...
    assert.ok(Object.isFrozen(compact)); assert.ok(compact.length > 2 && detailed.length <= 80, id);
    assert.ok(detailed.length >= compact.length); assert.ok(compact.some(op => op.surface), id);
