...
test('companion readiness is sampled at telemetry cadence, recovers after loss, and defaults to primary UI', t=>{
  ...
  assert.equal(bridge.presented,false);
  bridge.update(0);assert.equal(bridge.presented,false);assert.equal(published,0);
  available=true;bridge.update(249);assert.equal(polls,1);
  bridge.update(250);assert.equal(bridge.presented,true);assert.equal(published,1);
  available=false;bridge.update(500);assert.equal(bridge.presented,false);assert.equal(published,1);
  available=true;bridge.update(750);assert.equal(bridge.presented,true);assert.equal(published,2);
  bridge.dispose();assert.equal(bridge.presented,false);
  ...
  browser.update(1000);assert.equal(browser.presented,false);browser.dispose();
});
test('hidden upper minimap releases its combat hit region while portal remains usable',()=>{
  ...
  assert.equal(isGameUIPoint(portal.x+10,portal.y+10,width,height,null,false),true);
});
