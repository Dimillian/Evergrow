...
test('gambles select equipment categories at bounded town level and share real recipe validation',()=>{
 ...
 for(const kind of GAMBLE_KINDS){const sheet=wealthy(),q=quoteService(sheet,gambler,40,{type:'gamble',kind});assert.ok(q.ok);
  ...
  const before=structuredClone(sheet),plan=planService(sheet,gambler,40,q.quote);assert.ok(plan.ok);assert.deepEqual(sheet,before);
  ...
  const next=quoteService(plan.character,gambler,40,{type:'gamble',kind});assert.ok(next.ok);assert.notEqual(next.item.id,q.item.id);
 }
});
test('gambling charges nothing on full bags, insufficient funds, forged quotes or other services',()=>{
 const sheet=wealthy(),q=quoteService(sheet,gambler,1,{type:'gamble',kind:'weapon'});assert.ok(q.ok);
 ...
 const plan=planService(sheet,gambler,1,q.quote);assert.ok(plan.ok);assert.ok(save(plan.character),'gambled gear round trips with the normal save validator');
});
test('stash transfers exact gear between locations, prevents duplication, and saves its ownership',()=>{
 ...
 const q=quoteService(sheet,stash,1,{type:'store',bag:3});assert.ok(q.ok);const stored=planService(sheet,stash,1,q.quote);assert.ok(stored.ok);
...
test('storage and gambling retain live state until persistence succeeds and preserve it on failure',async()=>{
 ...
 for(const [npc,request]of [[stash,{type:'store',bag:0}],[gambler,{type:'gamble',kind:'weapon'}]] as const){
  const q=quoteService(p.character,npc,1,request);assert.ok(q.ok);const before=structuredClone(p.character);
