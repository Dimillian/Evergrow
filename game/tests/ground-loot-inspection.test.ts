...
test('ground inspection retains scroll access, switches a stationary comparison, and closes with its owner', t => {
  ...
  assert.doesNotMatch(tooltip.innerHTML, /ui-item-equipped-inline/);
  ...
  update(null); assert.match(tooltip.innerHTML, /ui-item-equipped-inline/, 'Shift compares the occupied other ring without moving the mouse');
  win.dispatchEvent(Object.assign(new Event('keyup'), { code: 'ShiftLeft', shiftKey: false }));
  update(null); assert.doesNotMatch(tooltip.innerHTML, /ui-item-equipped-inline/);
