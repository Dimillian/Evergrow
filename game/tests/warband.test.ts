...
test('orders respect sanctuary and home return, and damage buffs snapshot at each attack windup',()=>{
  ...
  assert.equal(goblin.state,'windup');assert.equal(goblin.attackDamage,goblin.damage*1.2);assert.ok(goblin.damage>=6);
  chief.state='dead';chief.hp=0;updateWarbands(sim.enemies,sim.player,open,FIXED_STEP);assert.equal(goblin.attackDamage,goblin.damage*1.2);
});
