...
export function validContents(v: unknown): v is LocationContents { return object(v) && (v.campWounds === undefined || validCampWounds(v.campWounds)) && validActors(v.actors) && validPickups(v.pickups) && Array.isArray(v.groundItems) && v.groundItems.length <= 96 && v.groundItems.every(i => object(i) && integer(i.id, 1) && point(i) && validItem(i.item) && validTreasureFlight(i.flight)) && Array.isArray(v.groundGold) && v.groundGold.length <= 128 && v.groundGold.every(i => object(i) && integer(i.id, 1) && point(i) && integer(i.amount, 1) && validTreasureFlight(i.flight) && number(i.age, 0, 10)) && Array.isArray(v.clearedCamps) && v.clearedCamps.every(id => text(id, 180)) && object(v.defeatedCampMembers) && Object.entries(v.defeatedCampMembers).every(([k, a]) => text(k, 180) && Array.isArray(a) && a.length <= 32 && a.every(id => text(id, 180))); }
