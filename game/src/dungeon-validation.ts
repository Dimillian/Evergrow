...
export function validActors(v: unknown): v is StoredActor[] { return Array.isArray(v) && v.length <= ENCOUNTER_RULES.hardPopulationCap && v.every(a => object(a) && typeof a.kind === 'string' && Object.hasOwn(ENEMY_DEFINITIONS, a.kind) && ['normal', 'veteran', 'elite'].includes(a.rank as string) && integer(a.level, 1, 1e6) && typeof a.biome === 'string' && Object.hasOwn(BIOMES, a.biome) && integer(a.seed, 0, 4294967295) && point(a) && number(a.homeX, -4e7, 4e7) && number(a.homeY, -4e7, 4e7) && number(a.hp, 0, scaledEnemyStats(a.kind as StoredActor['kind'], a.level as number, a.rank as StoredActor['rank']).maxHp) && (a.campId === undefined || text(a.campId, 180) && text(a.memberId, 180)) && (a.bossPhases === undefined || integer(a.bossPhases, 0, 3))); }
...
export function validContents(v: unknown): v is LocationContents { return object(v) && (v.campWounds === undefined || validCampWounds(v.campWounds)) && validActors(v.actors) && validPickups(v.pickups) && Array.isArray(v.groundItems) && v.groundItems.length <= 96 && v.groundItems.every(i => object(i) && integer(i.id, 1) && point(i) && validItem(i.item)) && Array.isArray(v.groundGold) && v.groundGold.length <= 128 && v.groundGold.every(i => object(i) && integer(i.id, 1) && point(i) && integer(i.amount, 1) && number(i.age, 0, 10)) && Array.isArray(v.clearedCamps) && v.clearedCamps.every(id => text(id, 180)) && object(v.defeatedCampMembers) && Object.entries(v.defeatedCampMembers).every(([k, a]) => text(k, 180) && Array.isArray(a) && a.length <= 32 && a.every(id => text(id, 180))); }
