...
export function executeService(player: Player, npc: TownNPC, world: WorldQuery, quote: ServiceQuote,
  persist: (character: CharacterSheet, hp: number, mana: number) => { ok: boolean; message?: string }): { ok: boolean; message: string } {
  ...
  const result = persist(plan.character, candidate.hp, candidate.mana);
