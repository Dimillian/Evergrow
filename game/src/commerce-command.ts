...
import { planService, type ServiceQuote, type ServiceResult } from './commerce.ts';
...
export async function executeService(player: Player, npc: TownNPC, world: WorldQuery, quote: ServiceQuote,
  persist: (character: CharacterSheet, hp: number, mana: number) => { ok: boolean; message?: string } | Promise<{ ok: boolean; message?: string }>): Promise<ServiceResult> {
