...
import { planBulkSale, type BulkSaleQuote } from './commerce-bulk.ts';
...
export type CommerceQuote = ServiceQuote | BulkSaleQuote;
export async function executeService(player: Player, npc: TownNPC, world: WorldQuery, quote: CommerceQuote,
  ...
  const plan = 'type' in quote ? planBulkSale(player.character, npc, player.level, quote) : planService(player.character, npc, player.level, quote);
