...
export function uniquePowerMarkup(unique: UniqueDefinition): string {
  ...
  return `<section class="ui-unique-power"><div class="ui-unique-power-skill"><span aria-hidden="true">${skillIconSVG(unique.skill, 28)}</span><strong>${escapeUI(skill.name)}</strong></div><p>${escapeUI(unique.power)}</p></section>`;
}
