...
export type EnemySpawn = Pick<Enemy, 'id' | 'kind' | 'level' | 'rank' | 'biome' | 'lootSeed' | 'x' | 'y'>
  & Partial<Pick<Enemy, 'lootIdentity' | 'rift' | 'dungeonTheme' | 'campId' | 'campMemberId'>> & { idleDuration: number };
...
export function createEnemy(s: EnemySpawn): Enemy {
  ...
  return {
    id:s.id, kind, level, rank, biome:s.biome, lootSeed, ...(s.lootIdentity?{lootIdentity:s.lootIdentity}:{}), ...(rift ? {rift} : {}), ...scaled, dungeonTheme:s.dungeonTheme,
