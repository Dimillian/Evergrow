/** Review-only decorative chassis, in a transparent 520 × 156 logical frame. */
const TAU = Math.PI * 2;
...
function actionTray(c: CanvasRenderingContext2D) {
  ...
  for (const side of [-1, 1]) {
    c.save(); c.translate(260, 0); c.scale(side, 1);
    c.beginPath(); c.moveTo(125, 82); c.bezierCurveTo(137, 85, 141, 99, 153, 104);
    c.lineTo(151, 112); c.bezierCurveTo(137, 107, 135, 91, 123, 89); c.closePath();
...
export function drawAstral(c: CanvasRenderingContext2D, time: number): void {
