...
function results() {
  ...
  if(!group&&!query){mount.innerHTML=`<section class="tools-overview"><div class="tools-grid">${WORKSPACES.filter(w=>w.id!=='archive').map(w=>`<a class="tool-card workspace-card" href="/tools/?group=${w.id}"><h3>${w.icon} ${w.name}<span>→</span></h3><p>${w.description}</p><small>${TOOLS.filter(t=>t.group===w.id).length} connected views · Open workspace</small></a>`).join('')}</div><p style="margin-top:24px">Search above to jump directly to any tool. Earlier design concepts live in <a href="/tools/?group=archive">Archive ↗</a>.</p></section>`;return;}
