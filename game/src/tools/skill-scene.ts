...
export interface SkillStudyOptions {scenario?:SkillStudyScenario;level?:number;baseline?:boolean;rear?:boolean;skill:SkillId;rank:number;specialization:string;weapon:string;facing:number;targets:'fan'|'line'|'ring'|'single'|'none';enemy:EnemyKind;x:number;y:number;}
...
export class SkillStudy {
  ...
  constructor(world:WorldQuery,options:SkillStudyOptions,loadout?:CharacterSheet){
    ...
    const distance=this.resolved.recipe.kind==='radial'?Math.min(range,this.resolved.recipe.radius*.6):range;
  ...
  step():CombatEvent[]{
    ...
    const cast=ready&&(!this.didCast||scenario==='sustain');
    const attack=(scenario==='followup'||scenario==='sustain')&&this.elapsed>=.55&&(this.didCast||!!this.options.baseline);
    ...
    if(scenario==='defense'&&this.elapsed>=this.nextHit){
      ...
      damagePlayer(amount,0,p.level,'physical',{player:p,world:{},random:()=>.99,emit:e=>events.push(e)});
    }
