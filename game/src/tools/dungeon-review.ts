...
const expedition=params.has('expedition');
...
const entranceFor=(seed:number):DungeonEntrance=>({id:'dungeon:review',name:dungeonTheme(seed,selectedTheme).name,theme:selectedTheme,...(expedition?{expedition:{attempt:1,stage:0,choice:0,modifier:'elite' as const}}:{}),seed,level:8,biome:'deadwood',x:0,y:0});
function floorMap(canvas:HTMLCanvasElement,seed:number,labels=true){
    const floor=generateDungeon(seed,8,entranceFor(seed)),run=createDungeonRun(entranceFor(seed)),c=canvas.getContext('2d')!,bounds=dungeonMapBounds(floor);
...
function render(){
    ...
    if(view==='gallery'){
        ...
        const composite=document.createElement('canvas');composite.width=1800;composite.height=2120;const cc=composite.getContext('2d')!;cc.fillStyle='#080e12';cc.fillRect(0,0,1800,2120);
        for(let i=0;i<6;i++){
            const n=(seed+i)>>>0,tid=DUNGEON_THEME_IDS[i],t=dungeonTheme(n,tid),oldTheme=selectedTheme;selectedTheme=tid;const f=generateDungeon(n,8,entranceFor(n));
            ...
            cc.save();cc.translate((i%3)*600,Math.floor(i/3)*1060);cc.drawImage(canvas,0,120);text(cc,t.name,28,35,1.5,t.accent);text(cc,`Seed ${n} · ${f.rooms.length} rooms · ${f.edges.length-f.rooms.length+1} loops`,28,78,1,'#9aacaf');
            text(cc,'◎ Entry   ◇ Encounter   ■ Treasure',28,970,.9,'#a9b9b5');
        ...
        const canvas=content.querySelector('canvas')!,c=canvas.getContext('2d')!,floor=generateDungeon(seed,8,entranceFor(seed)),run=createDungeonRun(entranceFor(seed));exportCanvas=canvas;
