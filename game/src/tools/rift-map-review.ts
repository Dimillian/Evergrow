...
export function mountRiftMapReview(root:HTMLElement,params:URLSearchParams):()=>void {
  ...
  let arrival=params.has('arrival'),scene=params.get('scene')==='pack'||arrival||animate;
  ...
  const draw=()=>{
    ...
    if(animate&&scene){image.hidden=true;display.hidden=false;let previous=performance.now();const frame=(now:number)=>{const delta=Math.min(.05,(now-previous)/1000);previous=now;if(!life.signal.aborted){if(!document.hidden)present?.(delta);animationFrame=requestAnimationFrame(frame);}};animationFrame=requestAnimationFrame(frame);}
    root.querySelector('[data-atmosphere]')!.textContent=animate?'Freeze atmosphere':'Animate atmosphere';
    (root.querySelector('[aria-label="Rift progress"]') as HTMLSelectElement).value=String(progress===0?0:progress<.75?.5:.9);image.alt=arrival?'Rift guardian arrival':scene?'Rift pack in the actual game world':'Complete generated open-world rift map';
    ...
    const url=new URL(location.href);url.searchParams.set('view','map');if(animate)url.searchParams.set('atmosphere','');else url.searchParams.delete('atmosphere');url.searchParams.set('progress',String(progress));url.searchParams.set('layout',layout);url.searchParams.set('encounter',encounter);url.searchParams.set('seed',String(seed));url.searchParams.set('biome',biome);if(arrival)url.searchParams.set('arrival','');else url.searchParams.delete('arrival');if(scene)url.searchParams.set('scene','pack');else url.searchParams.delete('scene');history.replaceState(null,'',url);
  };
  root.querySelector('[data-atmosphere]')!.addEventListener('click',()=>{animate=!animate;scene=true;arrival=false;draw();},{signal:life.signal});
  root.querySelector('[aria-label="Rift progress"]')!.addEventListener('change',e=>{progress=Number((e.target as HTMLSelectElement).value);scene=true;arrival=false;draw();},{signal:life.signal});
