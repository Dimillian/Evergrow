...
export function forgeProfiles(kind: ItemKind) {
  return kind==='charm'?CHARM_PROFILES:kind==='weapon'?WEAPON_PROFILES:kind==='shield'?SHIELD_PROFILES:kind==='ring'||kind==='amulet'?jewelryProfiles(kind):FOCUS_PROFILES.filter(p=>p.visual.kind===kind);
}
export function forgeMaterials(kind: ItemKind, profile: string) {return kind==='charm'?[]:itemMaterialPool(kind,WEAPON_PROFILES.find(p=>p.id===profile)?.family);}
