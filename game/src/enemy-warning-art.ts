...
export function enemyWarnings(e: Enemy, alpha = 1): Warning[] {
  ...
  const base = { x, y, angle: e.attackAngle, progress, locked: e.state === 'attack' || e.stateTime >= d.aimLock, color: '#f49b64' };
  if (e.kind === 'warden') {
  ...
  if (d.attack === 'ground') return [{ ...base, x: e.attackTargetX, y: e.attackTargetY, color: '#a3ceff', shape: { kind: 'circle', radius: d.blastRadius } }];
  if (d.attack === 'projectile') return d.shotOffsets.map(offset => ({ ...base, angle: e.attackAngle + offset,
    color: d.projectileStyle === 'arrow' ? '#ffc18d' : '#83eab5', shape: { kind: 'lane', length: d.projectile.speed * d.projectile.life, width: d.projectile.radius } }));
...
export function drawEnemyWarning(c: CanvasRenderingContext2D, e: Enemy, alpha: number, time: number, reduced: boolean): void {
  if (e.kind === 'warden' && e.bossMove === 'summon' && (e.state === 'windup' || e.state === 'attack'))
    drawGlow(c, e.x, e.y - 25, 70, '#abcdbb', .25 + Math.min(1, e.stateTime / Math.max(.01, e.stateDuration)) * .3);
  for (const w of enemyWarnings(e, alpha)) {
    c.save(); c.translate(w.x, w.y); c.rotate(w.angle);
    drawAttackWarning(c, w.shape, w.progress, w.color, time + e.id * .137, reduced, w.locked); c.restore();
  }
}
