...
export function sampleGearLight(x:number,y:number,lights:readonly PointLight[]):GearLight {
  let dx=-.45*.5,dy=-.6*.5,dz=.66*.5,weight=.5,red=233*.5,green=241*.5,blue=255*.5,total=0;
  ...
  if(!total)return DEFAULT_GEAR_LIGHT;
  return {direction:[dx/weight,dy/weight,dz/weight],color:`rgb(${Math.round(red/weight)},${Math.round(green/weight)},${Math.round(blue/weight)})`,power:Math.min(1.35,.65+total*.4)};
}
