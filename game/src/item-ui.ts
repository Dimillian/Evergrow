...
import { TIER_COLORS, TIER_NAMES, STAT_LABELS, itemModifiers, formatStatValue } from './items.ts';
...
const CHANGE_LABELS: Record<PreviewStat, string> = {
...
const PERCENT = new Set<PreviewStat>(['blockChance', 'blockReduction', 'critChance', 'critMultiplier', 'moveSpeedMultiplier',
...
export function itemSlotMarkup(item: Item, size = 44): string {
  return `${itemIconSVG(item, size)}<span class="ui-item-level">${number(item.itemLevel, 0)}</span><span class="ui-item-tier" aria-hidden="true">${'<i></i>'.repeat(TIER_RANK[item.tier])}</span>`;
}
...
export function itemTooltipMarkup(item: Item, view: ItemPresentation): string {
  ...
  if (!view.equipped) {
    ...
    if (!preview.ok) comparison = `<div class="ui-item-comparison is-loss">${escapeUI(preview.message)}</div>`;
      ...
      const changes = preview.changes.map(change => {
        const difference = change.after - change.before, percentage = PERCENT.has(change.key);
  ...
  return `<div class="ui-item-heading"><div><span class="ui-item-class"><span class="ui-rarity-badge" data-tier="${item.tier}"><span aria-hidden="true">${['I', 'II', 'III', 'IV', 'V'][TIER_RANK[item.tier] - 1]}</span>${escapeUI(TIER_NAMES[item.tier])}</span><span>${escapeUI(item.baseName)}</span></span><h4>${escapeUI(item.name)}</h4></div></div>
