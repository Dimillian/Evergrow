...
import type { FrameLimit } from './frame-limit.ts';
...
function optionsMarkup(frameLimit: boolean): string {
  return `<section class="system-option-group" aria-labelledby="options-audio"><h3 id="options-audio">Audio</h3>${audioControlsMarkup(true)}</section>
    ...
    </section>${frameLimit ? `<section class="system-option-group" aria-labelledby="options-performance"><h3 id="options-performance">Performance</h3>
      <label class="pause-option" for="frame-limit"><span>Frame limit</span><select id="frame-limit" data-frame-limit><option value="display">Display</option><option value="60">60 FPS</option><option value="30">30 FPS</option></select></label>
...
export class PauseSystemWindows {
  ...
  open(destination: SystemDestination): void {
    ...
    if (destination === 'changelog') {
      ...
      this.window.innerHTML = `<header class="ui-window-header"><h2 id="system-window-title" class="ui-title">${title}</h2><button type="button" class="ui-button ui-button--quiet ui-button--icon" data-system-close aria-label="Close ${title}">${uiIcon('close')}</button></header>
        <div class="system-window-body ui-scroll-area">${destination === 'options' ? optionsMarkup(!!this.actions.setFrameLimit) : destination === 'controls' ? controlsMarkup() : '<div data-system-rankings></div>'}</div>
  ...
  private bindOptions(signal: AbortSignal): void {
    ...
    const frameLimit = root.querySelector<HTMLSelectElement>('[data-frame-limit]');
    frameLimit?.addEventListener('change', () => {
      this.actions.setFrameLimit?.(frameLimit.value === '60' || frameLimit.value === '30' ? frameLimit.value : 'display');
      this.refresh();
    }, { signal });
  ...
  refresh(): void {
    ...
    const frameLimit = this.window?.querySelector<HTMLSelectElement>('[data-frame-limit]');
    if (frameLimit) frameLimit.value = this.actions.frameLimit?.() ?? 'display';
