...
import { newExpeditionRoute, expeditionChoices, EXPEDITION_RULES, EXPEDITION_MODIFIERS } from './expedition-route.ts';
...
export class ExpeditionPanel {
  ...
  open(state:Expeditions,level:number,worldSeed:number,tableId:string):void {
    ...
    const choices=expeditionChoices(route),locked=level<EXPEDITION_RULES.minimumLevel;
    const displayedRoute=existing?.status==='complete'?existing:route;
    ...
    this.element.innerHTML=`<header class="ui-window-header"><h2 class="ui-title" id="expedition-title">Expeditions</h2><span class="expedition-progress">${displayedRoute.cleared} / 10 cleared</span><button class="ui-button ui-button--icon" data-close aria-label="Close expeditions">×</button></header>
      <div class="expedition-body ui-scroll-area"><nav class="expedition-route" aria-label="Dungeon route">${Array.from({length:10},(_,i)=>`<div class="expedition-step ${i<displayedRoute.cleared?'is-cleared':i===displayedRoute.cleared?'is-current':''}" ${i===displayedRoute.cleared?'aria-current="step"':''}><span>${i<displayedRoute.cleared?'✓':i+1}</span><small>${i===9?'Grand chest':i<displayedRoute.cleared?'Cleared':i===displayedRoute.cleared?'Next':''}</small></div>`).join('')}</nav>
      ...
      <aside class="expedition-reward"><div class="expedition-chest-icon" aria-hidden="true">▱<span>✦</span></div><div><h3>Tenth dungeon · Grand chest</h3><p>${EXPEDITION_RULES.grandRewards} rewards · ${EXPEDITION_RULES.grandRarity.epic}% Epic · ${EXPEDITION_RULES.grandRarity.legendary}% Legendary · ${EXPEDITION_RULES.grandRarity.rare}% Rare per roll</p></div></aside></div>
    ...
    this.element.querySelectorAll<HTMLCanvasElement>('[data-gate]').forEach(canvas=>{const entry=choices[Number(canvas.dataset.gate)],c=canvas.getContext('2d')!;c.translate(240,204);c.scale(1.25,1.25);drawCryptGate(c,{...entry,x:0,y:0},0);});
