...
export class TerrainStream {
  ...
  update(seed: number, coordinates: readonly TerrainCoordinate[], wildernessOnly = false) {
    this.wildernessOnly = wildernessOnly;
    this.seed = seed;
    this.wanted.clear();
    for (const p of coordinates.slice(0, 256)) this.wanted.set(`${seed}:${wildernessOnly}:${p.x}:${p.y}`, p);
    for (const [key, tile] of this.tiles) if (!this.wanted.has(key)) { tile.bitmap.close(); this.tiles.delete(key); }
    this.pump();
  }
  get(x: number, y: number) { return this.tiles.get(`${this.seed}:${this.wildernessOnly}:${x}:${y}`); }
  private pump() {
    if (this.failed || this.pending) return;
    for (const [key, p] of this.wanted) if (!this.tiles.has(key)) {
      this.pending = { id: ++this.serial, key }; this.port.postMessage({ id: this.serial, seed: this.seed, wildernessOnly: this.wildernessOnly, ...p }); return;
    }
  }
