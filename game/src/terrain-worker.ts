...
interface Request { id: number; seed: number; wildernessOnly?: boolean; x: number; y: number; }
...
scope.onmessage = ({ data }) => {
  try {
    if (!world || world.seed !== data.seed || world.wildernessOnly !== !!data.wildernessOnly) { world?.dispose(); world = new World(data.seed, !!data.wildernessOnly); }
