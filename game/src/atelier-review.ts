...
offSelect.value = ['shield', 'dual'].includes(params.get('offhand') ?? '') ? params.get('offhand')! : 'none';
...
function poseAt(index: number, time: number): CharacterPose {
  const weapon = WEAPON_PROFILES.find(p => p.id === weaponSelect.value)!;
  const angle = Number(facingSelect.value) * Math.PI / 4;
  const attack = index === 2 ? (time * weapon.baseAttacksPerSecond) % 1 : 0;
  ...
  return { kind: 'player', angle, attackAngle: angle, time, moving: index === 1 ? 1 : 0,
    ...
    attackStart: BASIC_ATTACK_PHASES.activeStart, attackEnd: BASIC_ATTACK_PHASES.activeEnd,
    ...
    offHand: weapon.hands === 2 || offSelect.value === 'none' ? null : offSelect.value === 'shield'
      ? { kind: 'shield', visual: shield.visual } : { kind: 'weapon', visual: dagger.visual } };
}
function refresh() {
  elapsed = 0;
...
function render() {
  for (const [index, scene] of scenes.entries()) {
    ...
    drawHumanoid(ctx, poseAt(index, elapsed || (index === 2 ? .12 : 1.4))); ctx.restore();
  }
}
...
if (!disposed) {
  ...
  const tick = (now: number) => {
    if (motion.checked && !reduced.matches) { elapsed += previous ? Math.min(.05, (now - previous) / 1000) : 0; render(); }
