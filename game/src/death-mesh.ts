...
interface MeshFace { points:Vec3[];color:string;width:number;depth:number;details?:MeshFace[] }
export class DeathMesh {
  private readonly faces: MeshFace[]=[];
  private readonly surfaces=new WeakMap<Frame3,{front:MeshFace;back:MeshFace;depth:number;taper?:(x:number,z:number)=>number}>();
  private readonly facing: number;
  private readonly scale: number;
  constructor(facing: number, scale: number) { this.facing=facing;this.scale=scale; }
  visibleSide(frame:Frame3):1|-1 {
    const normal=frame.forward;
    return -normal[0]*Math.cos(this.facing)+normal[1]*Math.sin(this.facing)+normal[2]*.55>=0?1:-1;
  }
  ...
  solid(frame: Frame3,outline: readonly Point[],depth: number,color: string,taper?:(x:number,z:number)=>number): Frame3 {
    const back=outline.map(([x,z])=>at(frame,x,-depth/2*(taper?.(x,z)??1),z));
    const front=outline.map(([x,z])=>at(frame,x,depth/2*(taper?.(x,z)??1),z));
    ...
    this.face(b,mixColor(color,'#091912',.3));const backFace=this.faces[this.faces.length-1];
    this.face(f,color);const frontFace=this.faces[this.faces.length-1];
    ...
    // Attached engravings must receive the same floor correction as their part.
    const placed={...frame,origin:vadd(frame.origin,[0,0,lift])};
    this.surfaces.set(placed,{front:frontFace,back:backFace,depth,taper});
    return placed;
  }
  /** Paint on a rigid surface, preserving its floor lift and occlusion order. */
  detail(frame:Frame3,outline:readonly Point[],color:string,width=0,side=1):void {
    const surface=this.surfaces.get(frame);
    if(!surface) throw new Error('Death details require the placed solid frame');
    const parent=side>0?surface.front:surface.back;
    (parent.details??=[]).push({points:outline.map(([x,z])=>this.world(at(frame,x,side*surface.depth/2*(surface.taper?.(x,z)??1),z))),color,width:width*this.scale,depth:parent.depth});
  }
