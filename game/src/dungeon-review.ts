...
let fx: PostFX | undefined, world: DungeonWorld | undefined, disposed = false, frame = 0;
async function boot() {
    ...
    root.innerHTML = '<header class="layout-review-header"><h1>Rootbound Crypt</h1></header><nav class="layout-review-toolbar"><button data-view="entry">Entrance</button><button data-view="chamber">Burial chamber</button><button data-view="corridor">Passage</button><button data-view="boss">Hollow Warden</button><button data-view="map">Explored floor</button></nav><figure class="layout-review-figure layout-review-frame"><canvas class="layout-review-scene" aria-label="Frozen dungeon preview"></canvas></figure>';
    ...
    function draw(view: string) {
        ...
        if (view === 'map') {
            ...
            renderer.cameraY = y;
            present = dt => {
                renderer.render(sim, scene, dt, { phase: 'paused', reducedMotion: reducedMotion.matches, fps: 0, debug: false });
                fx!.render(renderer.canvas, 0);
                c.drawImage(output, 0, 0);
                c.save(); c.scale(2,2);
                renderer.renderUI(c,sim,scene,{phase:'paused',reducedMotion:reducedMotion.matches,fps:0,debug:false}); c.restore();
            };
            present(0);
        }
    ...
    // Presentation only: actors, combat, exploration and saves never advance.
    let previous = performance.now();
    const animate = (now: number) => {
        if (disposed) return;
        if (now - previous >= 1000 / 30) {
            if (!document.hidden && !reducedMotion.matches) present?.(Math.min(.05, (now - previous) / 1000));
            previous = now;
        }
...
function dispose() { disposed = true; cancelAnimationFrame(frame); abort.abort(); fx?.dispose(); world?.dispose(); }
