...
import { equipmentScorer } from './equipment-score.ts';
...
export function planBestEquipment(sheet: CharacterSheet, level: number, keepWeapon = false): BestEquipmentPlan {
  ...
  const treeBonuses = getTreeBonuses(sheet.allocatedNodes);
  let count = 0;
  for (const slot of EQUIPMENT_SLOTS) {
    if (slot === 'weapon' && keepWeapon) continue;
    const current = draft.equipped[slot];
    const score = equipmentScorer(draft, level, slot, treeBonuses);
    let bestScore = score(draft);
    let best: ReturnType<typeof planEquipmentChange> | undefined;
    ...
    for (const { item, index } of candidates) {
      ...
      const candidateScore = score({ ...draft, inventory: plan.inventory, equipped: plan.equipped, inventoryLayout: plan.inventoryLayout });
      if (candidateScore > bestScore + 1e-9) { best = plan; bestScore = candidateScore; }
    }
    if (best?.ok) {
      draft.inventory = best.inventory; draft.equipped = best.equipped; draft.inventoryLayout = best.inventoryLayout; count++;
    }
  }
  if (!count) return { ok: false, message: 'No equipment upgrades available.' };
...
export function equipBest(sheet: CharacterSheet, level: number, choice: EquipBestChoice = 'check'): ActionResult {
  ...
  return { ok: true, message: `Auto Equip: upgraded ${plan.count} equipment ${plan.count === 1 ? 'slot' : 'slots'}.` };
}
