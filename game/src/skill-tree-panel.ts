...
export class SkillTreePanel {
  ...
  private updateDetail(): void {
    ...
    this.detail.innerHTML = `<header class="skill-node-heading"><div class="skill-atlas-emblem" style="--star-color:${COLORS[node.domain]}">${skillNodeIconSVG(node, 48)}</div>
      ...
      <p class="skill-atlas-description">${escapeUI(node.description)}${node.skill && costs?.variant ? `</p><p class="skill-atlas-description">${escapeUI(costs.variant.description)}` : ''}</p>${node.specialization ? `<p class="skill-atlas-requirement">Unlocks a variant for <b>${escapeUI(owner!.name)}</b>. One active at a time.</p>` : node.improvement ? '<p class="skill-atlas-requirement">Applies to all variants of this skill.</p>' : ''}${node.specialization ? specializationPreviewMarkup(node.specialization, this.player.derived, this.player.character) : ''}${bonuses ? `<div class="skill-atlas-bonuses ui-well">${bonuses}</div>` : ''}
  ...
  private progressionControls(node: SkillNode, owned: boolean): string {
    ...
    return `<section class="skill-rank-controls ui-well"><strong>Rank ${learned} / ${max}${current.bonusRanks ? ` · +${current.bonusRanks} gear` : ''}</strong>
      ${next ? `<div class="skill-rank-preview"><span>Next rank</span><span>${next.recipe.kind === 'guard' && current.recipe.kind === 'guard' ? `${Math.round(current.recipe.reduction*100)}% → ${Math.round(next.recipe.reduction*100)}% blocked · ${Number(current.recipe.duration.toFixed(2))}s → ${Number(next.recipe.duration.toFixed(2))}s` : `${Math.round(current.damageMultiplier*100)}% → ${Math.round(next.damageMultiplier*100)}% damage`}</span><span>${current.mana} → ${next.mana} mana</span>${next.cooldown ? `<span>${next.cooldown.toFixed(2)}s cooldown</span>` : ''}</div>
      ...
      <details class="skill-variant-links"><summary>Specialization paths</summary>${variants.map(v=>`<button class="ui-button ui-button--quiet" data-node="${specializationNode(v.id)}">${v.name} ↗</button>`).join('')}</details>` : ''}
