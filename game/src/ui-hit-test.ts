...
export function isGameUIPoint(x: number, y: number, width: number, height: number, extra: {x:number;y:number;width:number;height:number}|null = null, navigationVisible = true, lootLog: {x:number;y:number;width:number;height:number}|null = null): boolean {
  ...
  return isHUDPoint(x, y, width, height) || (navigationVisible && sidebar)
    || !!lootLog && x >= lootLog.x && y >= lootLog.y && x <= lootLog.x + lootLog.width && y <= lootLog.y + lootLog.height;
}
