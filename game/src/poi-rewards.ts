...
export function eventRewards(site: EventRecord) {
  ...
  const count = site.kind === 'cursedChest' ? Math.min(10, Math.floor(site.wavesCleared / 2) + Number(site.wavesCleared>0)) : isTrialKind(site.kind)&&!['graveyard','standingStones'].includes(site.kind) ? 2 : site.kind === 'camp' || site.kind === 'graveyard' ? 1 : site.kind === 'caravan' && site.choice === 'goods' ? 2
    : site.kind === 'reliquary' && random(1) < .25 ? 1 : 0;
  const veteran = isTrialKind(site.kind), weights = ENEMY_LOOT_TABLES[veteran ? 'veteran' : 'normal'].tierWeights;
  const items = Array.from({ length: count }, (_, i) => {
    ...
    const item = generateItem(siteHash(site.seed, i, 497), Math.min(1e6, site.level + Number(veteran)), kind, site.kind==='corruptedGrove'&&kind==='weapon'?'ember-staff':undefined, tier, material, {level:site.level,encounter:isTrialKind(site.kind)||site.kind==='camp'?'event':undefined});
    item.id = `poi:${site.id}:${i}`;
    return item;
  });
  const range = site.kind === 'cursedChest' ? [site.wavesCleared*10,site.wavesCleared*15] : isTrialKind(site.kind)&&!['graveyard','standingStones'].includes(site.kind) ? [20,35] : site.kind === 'camp' ? [8, 14] : site.kind === 'reliquary' ? [4, 8] : site.kind === 'caravan' && site.choice === 'coin' ? [22, 34] : [0, 0];
