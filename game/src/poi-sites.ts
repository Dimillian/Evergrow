...
import { siteHash, type WildernessSite } from './wilderness-sites.ts';
import { mainPathX } from './road-shape.ts';
...
export function queryEventSites(world: SiteWorld, x: number, y: number, w: number, h: number): EventSite[] {
  if (![x, y, w, h].every(Number.isFinite) || w <= 0 || h <= 0 || w > 100000 || h > 100000)
    return [];
  const sites = world.getWildernessSites(x - 220, y - 220, w + 440, h + 440).map(eventSite);
  const first = Math.floor((y - 120) / 900), last = Math.floor((y + h + 120) / 900);
  for (let row = first; row <= last; row++) {
    const seed = siteHash(row, 0, world.seed, 6531), sy = row * 900 + 480;
    const sx = mainPathX(sy) + ((seed & 1) ? -1 : 1) * 84;
    if (Math.hypot(sx, sy) < 420 || world.isSanctuary(sx, sy) || world.blocked(sx, sy, 25)
      || world.getWildernessSites(sx - 330, sy - 330, 660, 660).some(s => Math.hypot(s.x - sx, s.y - sy) < s.radius + 300))
      continue;
    sites.push({ id: `reliquary:${world.seed}:${row}`, kind: 'reliquary', name: 'Roadside reliquary', x: sx, y: sy, seed, biome: world.sampleBiome(sx, sy).id, level: getZoneAt(sx, sy).level });
  }
