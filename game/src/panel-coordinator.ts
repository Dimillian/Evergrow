...
export class PanelCoordinator {
  ...
  resume(): boolean {
    ...
    const next = this.current === 'map' && this.mapReturn ? this.mapReturn : this.current === 'chronicle' || this.current === 'lootLog' ? this.characterChildReturn : this.current !== 'paused' && this.returnToPause ? 'paused' : 'playing';
