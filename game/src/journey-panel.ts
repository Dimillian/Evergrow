...
export class JourneyPanel {
  ...
  update(state:JourneyState,facts:JourneyFacts,playing:boolean,width:number,height:number){
    ...
    const html=`<header><button class="journey-mini-title" data-open>Journeys<kbd class="hud-sidebar-key">J</kbd></button><button data-collapse aria-label="${state.collapsed?'Expand':'Collapse'} journeys" aria-expanded="${!state.collapsed}">${uiIcon('chevron')}</button></header>${state.collapsed?'':list.map(g=>`${g.id===recommended?.id?'<div class="journey-mini-section">Recommended</div>':g.id===firstNearby?'<div class="journey-mini-section">Nearby</div>':''}<button class="journey-mini-row ${g.id===guided?.id?'is-tracked':''}" data-goal="${e(g.id)}" data-tooltip="${e(g.name)} · ${e(journeyObjective(g,facts))} · Level ${g.level}${g.level>facts.level+2?' · Harder':''}" data-tooltip-align="end" aria-label="${e(g.name)} · Level ${g.level} · ${journeyLevelFit(g.level,facts.level)}"><span class="journey-symbol">${g.finishedAt!==undefined?'✓':g.id===guided?.id?'◆':'◇'}</span><span><strong>${e(g.name)}</strong></span><span class="journey-mini-level" data-fit="${journeyLevelFit(g.level,facts.level)}"><span>Lv</span> ${g.level}</span></button>`).join('')}`;
  ...
  private render(){
    ...
    const rows=(title:string,items:JourneyGoal[])=>items.length?`<h3>${title}</h3>${items.map(v=>`<button class="journey-list-row ${v.id===g?.id?'is-selected':''}" data-select="${e(v.id)}" aria-pressed="${v.id===g?.id}"><span>${v.finishedAt!==undefined?'✓':v.id===guided?.id?'◆':'◇'}</span><span>${e(v.name)}</span><span class="journey-list-level" data-fit="${journeyLevelFit(v.level,facts.level)}" aria-label="Level ${v.level} · ${journeyLevelFit(v.level,facts.level)}">${v.level}</span></button>`).join('')}`:'';
