...
The local slice now includes connected Deadwood, Verdant Forest, and swamp terrain; procedural settlements with enterable buildings; and exploration maps. Continue to prioritize the procedural asset engine, smooth transitions, readable navigation, and satisfying combat. The first character foundation now includes equipment, inventory, attributes, a large skill atlas, six unlockable active skills, and enemy gear drops. Trading, crafting, off-hand equipment, character saving, and deeper item/skill content remain future work. Read `docs/character-systems.md` before extending these systems.

The Astral skill row contains basic attack (LMB) and exactly five assignable skills (RMB, 1–4), empty on a new run. Potion (Q) and dodge (Space) occupy separate utility shortcuts beside the menu rail. Unassigned controls do nothing. Six major tree nodes unlock actual skills; the player assigns unlocked skills to the five slots. There is no default player fireball or universal right-click casting. Ember Lance is an explicitly unlocked and assigned skill. Wand weapon attacks remain future work.
...
- `game/src/progression.ts`: run-local level and XP thresholds/overflow. `character.ts` awards one skill point and five attribute points per gained level; simulation awards authored XP once per actual enemy death. Level gains do not auto-spend points or refill resources. Character progress is not persisted.
...
- `game/src/inventory.ts`: validated equip/unequip/swap, inventory insertion, and attribute allocation. The 48 bag cells and ten equipment slots are the character sheet's source of truth; failed actions never lose items.
...
- `game/src/equipment.ts`: shared starter/unarmed profiles and bounded weapon attack derivation. The character sheet owns gear; player render/combat equipment is a refreshed projection.
- `game/src/skill-tree.ts`: immutable graph of 2,788 stars, 2,878 curved connections, and 150 irregular themed constellations across Might, Cunning, and Arcana. Three winding arteries and sparse crosslinks support hybrid routes. Stable IDs, actual geometry bounds, validated connected allocation, bonus aggregation, and skill unlock lookup belong here. Six early majors unlock active skills; each constellation develops one authored stat specialty.
...
- `game/src/skill-content.ts` and `skill-combat.ts`: shared active-skill metadata/icons and six executable actions. Combat/HUD/tree consume shared costs, cooldowns, and potency. Skills require allocation and assignment; cooldowns belong to skill IDs.
...
- `game/src/item-art.ts` and `loot-art.ts`: code-defined equipment icons/worn materials and world loot markers with native-resolution labels. Simulation owns drops and proximity pickup; drawing cannot award or remove items.
