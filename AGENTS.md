...
- `game/src/inventory.ts`: validated equip/unequip/swap, transactional stowing for hand conflicts, inventory insertion, and attribute allocation. The 48 bag cells and eleven equipment slots are the character sheet's source of truth; failed actions never lose items.
...
- `game/src/skill-tree.ts`: immutable graph of 2,824 stars, 2,923 curved connections, and 150 irregular themed constellations across Might, Cunning, and Arcana. Three winding arteries and sparse crosslinks support hybrid routes. Stable IDs, actual geometry bounds, validated connected allocation, bonus aggregation, and skill unlock lookup belong here. Seventeen majors in nine weapon schools unlock active skills at four-point first paths and seven-point advanced paths; each constellation develops one authored stat specialty.
...
- `game/src/skill-content.ts` and `skill-combat.ts`: shared active-skill metadata/icons, compatible-weapon selection, and seventeen executable actions. Combat/HUD/tree consume shared equipment requirements, costs, cooldowns, and potency. Skills require allocation and assignment; cooldowns belong to skill IDs. Melee skills use a compatible held weapon, preferring main hand; staff/bow skills use their two-handed main weapon and shield skills require an equipped shield.
- `game/src/projectile-combat.ts` and `projectile-art.ts`: swept projectile contacts and bounded pierce/chain/explosion payloads; separate procedural arrow/elemental presentation. Simulation owns delayed ground effects, continuous skill dashes, blocks, slows, and burns. Siphon heals from actual direct-hit life removed; burns tick every 0.5 seconds without crit or life on hit.
